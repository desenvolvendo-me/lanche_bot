# frozen_string_literal: true

require "rails_helper"

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to test the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator. If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails. There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.

RSpec.describe "/lunches", type: :request do
  # Lunch. As you add validations to Lunch, be sure to
  # adjust the attributes here as well.
  let(:valid_attributes) do
    skip("Add a hash of attributes valid for your model")
  end

  let(:invalid_attributes) do
    skip("Add a hash of attributes invalid for your model")
  end

  describe "GET /index" do
    it "renders a successful response" do
      Lunch.create! valid_attributes
      get lunches_url
      expect(response).to be_successful
    end
  end

  describe "GET /show" do
    it "renders a successful response" do
      lunch = Lunch.create! valid_attributes
      get lunch_url(lunch)
      expect(response).to be_successful
    end
  end

  describe "GET /new" do
    it "renders a successful response" do
      get new_lunch_url
      expect(response).to be_successful
    end
  end

  describe "GET /edit" do
    it "render a successful response" do
      lunch = Lunch.create! valid_attributes
      get edit_lunch_url(lunch)
      expect(response).to be_successful
    end
  end

  describe "POST /create" do
    context "with valid parameters" do
      it "creates a new Lunch" do
        expect do
          post lunches_url, params: { lunch: valid_attributes }
        end.to change(Lunch, :count).by(1)
      end

      it "redirects to the created lunch" do
        post lunches_url, params: { lunch: valid_attributes }
        expect(response).to redirect_to(lunch_url(Lunch.last))
      end
    end

    context "with invalid parameters" do
      it "does not create a new Lunch" do
        expect do
          post lunches_url, params: { lunch: invalid_attributes }
        end.to change(Lunch, :count).by(0)
      end

      it "renders a successful response (i.e. to display the 'new' template)" do
        post lunches_url, params: { lunch: invalid_attributes }
        expect(response).to be_successful
      end
    end
  end

  describe "PATCH /update" do
    context "with valid parameters" do
      let(:new_attributes) do
        skip("Add a hash of attributes valid for your model")
      end

      it "updates the requested lunch" do
        lunch = Lunch.create! valid_attributes
        patch lunch_url(lunch), params: { lunch: new_attributes }
        lunch.reload
        skip("Add assertions for updated state")
      end

      it "redirects to the lunch" do
        lunch = Lunch.create! valid_attributes
        patch lunch_url(lunch), params: { lunch: new_attributes }
        lunch.reload
        expect(response).to redirect_to(lunch_url(lunch))
      end
    end

    context "with invalid parameters" do
      it "renders a successful response (i.e. to display the 'edit' template)" do
        lunch = Lunch.create! valid_attributes
        patch lunch_url(lunch), params: { lunch: invalid_attributes }
        expect(response).to be_successful
      end
    end
  end

  describe "DELETE /destroy" do
    it "destroys the requested lunch" do
      lunch = Lunch.create! valid_attributes
      expect do
        delete lunch_url(lunch)
      end.to change(Lunch, :count).by(-1)
    end

    it "redirects to the lunches list" do
      lunch = Lunch.create! valid_attributes
      delete lunch_url(lunch)
      expect(response).to redirect_to(lunches_url)
    end
  end
end
